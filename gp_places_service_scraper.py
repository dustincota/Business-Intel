#!/usr/bin/env python3
"""Shared Google Places scraper for service-business datasets."""
...
def classify_hvac(name):
    lowered = name.lower()
    if any(keyword in lowered for keyword in ("refrigeration", "cooler", "freezer")):
        return "refrigeration"
    if any(keyword in lowered for keyword in ("duct", "vent", "air duct")):
        return "ductwork"
    if any(keyword in lowered for keyword in ("indoor air", "iaq", "air quality", "filtration")):
        return "indoor_air_quality"
    if any(keyword in lowered for keyword in ("heating", "furnace", "heat pump", "boiler")):
        return "heating"
    if any(keyword in lowered for keyword in ("air conditioning", "a/c", "ac ", "cooling")):
        return "air_conditioning"
    if "commercial" in lowered:
        return "commercial"
    if any(keyword in lowered for keyword in ("residential", "home service", "home services")):
        return "residential"
...
def classify_plumbing(name):
    lowered = name.lower()
    if any(keyword in lowered for keyword in ("drain", "rooter", "clog", "sewer")):
        return "drain_sewer"
    if any(keyword in lowered for keyword in ("septic", "lift station")):
        return "septic"
    if any(keyword in lowered for keyword in ("water heater", "tankless")):
        return "water_heater"
    if any(keyword in lowered for keyword in ("leak", "repiping", "repipe", "pipe")):
        return "pipe_leak"
    if any(keyword in lowered for keyword in ("commercial", "industrial")):
        return "commercial"
    if any(keyword in lowered for keyword in ("residential", "home service", "home services")):
        return "residential"
    if any(keyword in lowered for keyword in ("emergency", "24/7", "24 hour")):
...
SERVICE_CONFIGS = {
    "hvac": {
        "table_name": "hvac_companies",
        "subtype_field": "hvac_type",
        "default_state": "FL",
        "brands": {
            "one hour heating": "One Hour Heating & Air Conditioning",
            "one hour air": "One Hour Heating & Air Conditioning",
            "aire serv": "Aire Serv",
            "del-air": "Del-Air",
            "cool today": "Cool Today",
        },
        "search_specs": [
            {"keyword": "hvac contractor", "label": "hvac_contractor"},
            {"keyword": "air conditioning contractor", "label": "air_conditioning_contractor"},
            {"keyword": "heating contractor", "label": "heating_contractor"},
        ],
        "classifier": classify_hvac,
    },
    "plumbing": {
        "table_name": "plumbing_companies",
        "subtype_field": "plumbing_type",
        "default_state": "FL",
        "brands": {
            "roto-rooter": "Roto-Rooter",
            "mr. rooter": "Mr. Rooter",
            "benjamin franklin plumbing": "Benjamin Franklin Plumbing",
            "rescue rooter": "Rescue Rooter",
        },
        "search_specs": [
            {"type": "plumber", "label": "plumber"},
        ],
        "classifier": classify_plumbing,
    },
}


def build_paths(service_key, state_code):
    return {
        "log_file": os.path.join(SCRIPT_DIR, f"{service_key}_{state_code.lower()}.log"),
        "progress_file": os.path.join(
            SCRIPT_DIR, f"{service_key}_{state_code.lower()}_progress.json"
        ),
        "seen_file": os.path.join(SCRIPT_DIR, f"{service_key}_{state_code.lower()}_seen.json"),
    }
...
def load_progress(progress_file, state_code):
...
def nearby_search(lat, lon, search_spec, session, log_file, page_token=None):
    url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json"
    params = {"key": GOOGLE_API_KEY, "radius": SEARCH_RADIUS}

    if "type" in search_spec:
        params["type"] = search_spec["type"]
    if "keyword" in search_spec:
        params["keyword"] = search_spec["keyword"]
    ...
    for _attempt in range(3):
        try:
            ...
            if status == "OVER_QUERY_LIMIT":
                log("    Rate limited by Google Places, waiting 60s...", log_file)
                time.sleep(60)
                continue
...
def get_place_details(place_id, session, log_file):
    ...
    for _attempt in range(3):
        try:
            ...
            if data.get("status") == "OVER_QUERY_LIMIT":
                log("    Details rate limited, waiting 60s...", log_file)
                time.sleep(60)
                continue
        except Exception:
            time.sleep(2)
...
def detect_brand(name, config):
    lowered = name.lower()
    for keyword, brand in config["brands"].items():
        if keyword in lowered:
            return brand
    return None


def place_to_record(place, details, search_spec, config, state_code):
    ...
    return {
        "name": name,
        "brand": detect_brand(name, config),
        subtype_field: config["classifier"](name),
        ...
        "source_state": state_code,
        "source_query": search_spec["label"],
        "data_source": "google_places",
    }


def upsert_batch(records, table_name, session, log_file):
    ...
    for _attempt in range(3):
        try:
            response = session.post(url, headers=headers, json=records, timeout=60)
            ...
            log(f"    Upsert error: {response.status_code} {response.text[:300]}", log_file)
            ok = 0
            for record in records:
                try:
                    fallback = session.post(url, headers=headers, json=[record], timeout=15)
...
def run_service_scraper(service_key, state_code=None):
    if service_key not in SERVICE_CONFIGS:
        supported = ", ".join(sorted(SERVICE_CONFIGS))
        sys.exit(f"Unsupported service: {service_key}. Expected one of: {supported}")

    config = SERVICE_CONFIGS[service_key]
    state_code = (state_code or config["default_state"]).upper()

    if state_code not in STATE_BOXES:
        sys.exit(f"Unsupported state code: {state_code}")
    if not GOOGLE_API_KEY or not SUPABASE_URL or not SUPABASE_KEY:
        sys.exit("Missing Google or Supabase credentials.")

    paths = build_paths(service_key, state_code)
    progress = load_progress(paths["progress_file"], state_code)
    seen = load_seen(paths["seen_file"])
    session = requests.Session()
    grid = generate_state_grid(state_code)
    ...
    log("=" * 60, paths["log_file"])
    log(
        f"GOOGLE PLACES -> {config['table_name']} ({state_code})",
        paths["log_file"],
    )
    log("=" * 60, paths["log_file"])
    log(
        f"  Grid: {len(grid)} points | Resuming from {start_idx}",
        paths["log_file"],
    )
    log(
        f"  Found so far: {total_found} | Inserted: {total_inserted} | API calls: {api_calls}",
        paths["log_file"],
    )
    log(f"  Seen place_ids: {len(seen)}", paths["log_file"])
    ...
    for idx in range(start_idx, len(grid)):
        lat, lon = grid[idx]
        log(
            f"[{idx + 1}/{len(grid)}] Scanning {lat}, {lon}",
            paths["log_file"],
        )

        for search_spec in config["search_specs"]:
            results, next_page_token = nearby_search(
                lat, lon, search_spec, session, paths["log_file"]
            )
            api_calls += 1
            pages = 1
            ...
            while True:
                for place in results:
                    ...
                    seen.add(place_id)
                    details = get_place_details(place_id, session, paths["log_file"])
                    api_calls += 1
                    record = place_to_record(
                        place, details, search_spec, config, state_code
                    )
                    if not record:
                        continue
                    ...
                    if len(batch) >= BATCH_SIZE:
                        ...
                        log(
                            f"    Upserted {inserted}/{len(batch)} | Total inserted: {total_inserted}",
                            paths["log_file"],
                        )
                        batch = []
                        save_seen(seen, paths["seen_file"])
        ...
        save_progress(progress, paths["progress_file"])
    ...
    if batch:
        ...
        total_inserted += inserted
        log(
            f"Final upsert: {inserted}/{len(batch)} | Total inserted: {total_inserted}",
            paths["log_file"],
        )
    ...
    log("Done.", paths["log_file"])


def main(service_key):
    state_code = sys.argv[1].upper() if len(sys.argv) > 1 else None
    run_service_scraper(service_key, state_code)
